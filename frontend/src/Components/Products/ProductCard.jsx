...
const ProductCard = ({ product, flex, description, renderAdd, smallImg }) => {
  ...
  return (
    <>
      <div
        className={`w-[200px] shadow-[0_0_10px_rgba(0,0,0,0.2)] p-[2px] relative text-black no-underline group ${
          flex
            ? "shadow-none h-[35vh] flex flex-col md:flex-row md:gap-[50px] w-full"
            : ""
        }`}
      >
        <Link to={`/products/${product.id}`}>
          <img
            src={product.image}
            alt="product image"
            className={`p-[15px] w-[100%] contain-content h-[200px] ${
